...
def double_q_learning(
    ...
    Q_table1 = defaultdict(lambda: inf_Q*np.ones(num_actions))
    Q2_table1 = defaultdict(lambda: inf_Q*np.ones(num_actions))
    Q_nvisits1 = defaultdict(lambda: np.zeros(num_actions))
    
    Q_table2 = defaultdict(lambda: inf_Q*np.ones(num_actions))
    Q2_table2 = defaultdict(lambda: inf_Q*np.ones(num_actions))
    Q_nvisits2 = defaultdict(lambda: np.zeros(num_actions))
    ...
    for i_step in tqdm(
        ...
        if np.random.rand() < 0.5:
            if not terminated:
                action_muhats = copy.deepcopy(Q_table1[new_state])
                action_nvisits = Q_nvisits1[new_state]
                action_muhats[action_nvisits == 0] = -inf_mu
                Q_est = np.max(action_muhats) if np.sum(action_nvisits) != 0 else 0
            ...
            Q_nvisits1[cur_state][action] += 1
            lr = lr_sched_fn(Q_nvisits1[cur_state][action])
            
            Q_table1[cur_state][action] = \
                (1-lr)*Q_table1[cur_state][action] + lr*td_target
            
            Q2_table1[cur_state][action] = \
                (1-lr)*Q2_table1[cur_state][action] + lr*td_target**2

        else:
            if not terminated:
                action_muhats = copy.deepcopy(Q_table2[new_state])
                action_nvisits = Q_nvisits2[new_state]
                action_muhats[action_nvisits == 0] = -inf_mu
                Q_est = np.max(action_muhats) if np.sum(action_nvisits) != 0 else 0
            ...
            Q_nvisits2[cur_state][action] += 1
            lr = lr_sched_fn(Q_nvisits2[cur_state][action])
            
            Q_table2[cur_state][action] = \
                (1-lr)*Q_table2[cur_state][action] + lr*td_target
            
            Q2_table2[cur_state][action] = \
                (1-lr)*Q2_table2[cur_state][action] + lr*td_target**2
                
        Q_table[cur_state][action] = (Q_table1[cur_state][action] + Q_table2[cur_state][action])/2
        Q2_table[cur_state][action] = (Q2_table1[cur_state][action] + Q2_table2[cur_state][action])/2
        Q_nvisits[cur_state][action] = Q_nvisits1[cur_state][action] + Q_nvisits2[cur_state][action]
        ...
        stats.append((np.sum(Q_nvisits[start_state]), reward, Q_start_est))
...
def create_q_algo(algo_name, **args):
    if algo_name == "MEE" or algo_name == "q_learning":
        q_algo = q_learning
    elif algo_name == "avg" or algo_name == "avg_q_learning":
        q_algo = avg_q_learning
    elif algo_name == "DE" or algo_name == "double_q_learning":
        q_algo = double_q_learning
    elif algo_name == "haver0" or algo_name == "haver_q_learning":
        q_algo = haver_q_learning
    elif algo_name == "Haver" or algo_name == "haver3_q_learning":
        q_algo = haver3_q_learning
    elif algo_name == "WE" or algo_name == "weightedms_q_learning":
